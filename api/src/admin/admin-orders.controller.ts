// src/admin/admin-orders.controller.ts — PostgreSQL rewrite, no Mongoose
import {
...
import {
  IsIn, IsString, IsOptional, MaxLength, IsUUID, IsNumberString,
...
// ── Request DTOs — validated by NestJS ValidationPipe ────────────────────────

class UpdateOrderStatusDto {
  @ApiProperty({ enum: VALID_STATUSES, description: 'New order status' })
  @IsIn(VALID_STATUSES, { message: `status must be one of: ${VALID_STATUSES.join(', ')}` })
  status: OrderStatus

  @ApiProperty({ required: false, maxLength: 500, description: 'Optional status note for timeline' })
  @IsOptional()
  @IsString()
...
class AddTrackingDto {
  ...
  @IsString()
  @MaxLength(200, { message: 'trackingNumber must not exceed 200 characters' })
  trackingNumber: string

  @ApiProperty({ required: false, description: 'ISO 8601 estimated delivery date' })
  @IsOptional()
  @IsString()
  @MaxLength(50)
...
export class AdminOrdersController {
  ...
  async updateStatus(
    ...
    // status is validated by @IsIn(VALID_STATUSES) on UpdateOrderStatusDto via NestJS ValidationPipe.
    const order = await this.db.queryOne<any>(
    ...
    // Determine payment status for terminal states
    ...
    // Shipping email — only if not already sent
  ...
  async addTracking(
    ...
    if (!wasShipped) {
      timeline.push({
        ...
        note: `Shipped — tracking: ${body.trackingNumber}`,
      })
    }
