...
export class CartService {
  ...
  private async getOrCreate(sessionId: string) {
    ...
    if (!cart) {
      cart = await this.db.queryOne<any>(
        `INSERT INTO store.carts (session_id) VALUES ($1) RETURNING *`,
        [sessionId],
      )
    }
    return cart
  ...
  private computeSummary(items: any[], coupon: any) {
    ...
    // BUG FIX: itemCount = total units (sum of quantities), not unique product slots
  ...
  async getCart(sessionId: string) {
    ...
    return {
      ...cart,
      items: cart.items ?? [],
      summary: this.computeSummary(cart.items ?? [], cart.coupon),
    }
  }
  ...
  async addItem(dto: AddToCartDto & { sessionId: string }, userId?: string) {
    ...
    const cart = await this.getOrCreate(sessionId)
    const product =
      this.products.findProductByAsin(dto.productId) ??
      this.products.findProductBySlug(dto.productId)
  ...
  async updateItem(dto: UpdateCartDto & { sessionId: string }) {
    ...
    const cart = await this.getOrCreate(sessionId)
    const items: any[] = cart.items ?? []
    // BUG FIX: match variantId exactly — prevents updating wrong variant
    ...
    const updated = await this.db.queryOne<any>(
      'UPDATE store.carts SET items=$1,updated_at=NOW() WHERE session_id=$2 RETURNING *',
  ...
  async removeItem(dto: { productId: string; variantId?: string; sessionId: string }) {
    const { sessionId } = dto
    const cart = await this.getOrCreate(sessionId)
    // BUG FIX: match variantId — without this, removing size-S also deletes size-L
    const items = (cart.items ?? []).filter((i: any) => {
      if (i.productId !== dto.productId) return true
      if (dto.variantId == null) return false
      return i.variantId !== dto.variantId
    })
    const updated = await this.db.queryOne<any>(
      'UPDATE store.carts SET items=$1,updated_at=NOW() WHERE session_id=$2 RETURNING *',
      [JSON.stringify(items), sessionId],
  ...
  async bulkRemoveItems(dto: { items: BulkRemoveItemDto[]; sessionId: string }) {
    const { sessionId, items: toRemove } = dto
    const cart = await this.getOrCreate(sessionId)
    const items = (cart.items ?? []).filter((cartItem: any) =>
      !toRemove.some(
        (r) =>
          r.productId === cartItem.productId &&
          (r.variantId == null || r.variantId === cartItem.variantId),
      ),
    )
    ...
    return {
      ...updated,
      summary: this.computeSummary(items, updated?.coupon),
      removedCount,
    }
  }
  ...
  async clearCart(sessionId: string) {
    await this.db.execute(
      'UPDATE store.carts SET items=$1,coupon=NULL,updated_at=NOW() WHERE session_id=$2',
  ...
  async applyCoupon(dto: ApplyCouponDto & { sessionId: string }, userId: string | null) {
    ...
    if (!coupon)
      throw new BadRequestException({ code: 'INVALID_COUPON', message: 'Invalid or expired coupon' })
    if (coupon.used_count >= coupon.usage_limit)
      throw new BadRequestException({ code: 'COUPON_EXHAUSTED' })
    if (userId && coupon.used_by_user_ids.includes(userId))
      throw new BadRequestException({ code: 'COUPON_ALREADY_USED' })
    ...
    const updated = await this.db.queryOne<any>(
      'UPDATE store.carts SET coupon=$1,updated_at=NOW() WHERE session_id=$2 RETURNING *',
      [JSON.stringify(coupon), sessionId],
    )
  ...
  async removeCoupon(sessionId: string) {
    const updated = await this.db.queryOne<any>(
      'UPDATE store.carts SET coupon=NULL,updated_at=NOW() WHERE session_id=$1 RETURNING *',
      [sessionId],
    )
  ...
  async mergeCart(dto: MergeCartDto & { userId: string }) {
    const { userId } = dto
    const guest = await this.db.queryOne<any>(
      'SELECT * FROM store.carts WHERE session_id=$1',
      [dto.guestSessionId],
    )
    if (!guest || !(guest.items ?? []).length) return { merged: false }

    const userSessionId = `user:${userId}`
    let userCart = await this.db.queryOne<any>(
      'SELECT * FROM store.carts WHERE session_id=$1',
      [userSessionId],
    )
    ...
    for (const item of userCart.items ?? []) {
      itemMap.set(`${item.productId}::${item.variantId ?? ''}`, { ...item })
    }
  ...
  async getSummary(sessionId: string) {
    const cart = await this.db.queryOne<any>(
      'SELECT items, coupon FROM store.carts WHERE session_id=$1',
      [sessionId],
    )
  ...
  async getCartByUser(userId: string) {
    const cart = await this.db.queryOne<any>(
      'SELECT * FROM store.carts WHERE user_id=$1',
      [userId],
    )
    if (!cart) return null
