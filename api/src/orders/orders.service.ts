...
export class OrdersService {
  ...
  private async nextOrderId(): Promise<string> {
    const row = await this.db.queryOne<{ nextval: string }>(
      'SELECT nextval($1) AS nextval', ['store.order_seq'],
    )
  ...
  async create(userId: string, dto: CreateOrderDto) {
    ...
    if (dto.idempotencyKey) {
      ...
      if (existing) {
        ...
        const order = await this.db.queryOne<any>(
          'SELECT * FROM store.orders WHERE order_id=$1', [existing],
        )
    ...
    for (const item of cartData.items) {
      if (!this.products.findProductByAsin(item.asin)) {
        throw new BadRequestException({ code: 'PRODUCT_NOT_FOUND', message: `Product ${item.asin} not found` })
      }
    }
    ...
    const order = await this.db.transaction(async (client) => {
      ...
      if (cartData.coupon) {
        ...
        if (!couponRow) {
          throw new BadRequestException({ code: 'COUPON_INVALID', message: 'Coupon is no longer valid' })
        }
    ...
    await this.webhooks.dispatch('order.created', { orderId, userId, total: summary.total }).catch(() => void 0)
  ...
  async findUserOrders(userId: string, page = 1, limit = 10) {
    ...
    const [rows, countRow] = await Promise.all([
      ...
      this.db.queryOne<{ cnt: number }>(
        'SELECT COUNT(*)::int AS cnt FROM store.orders WHERE user_id=$1', [userId],
      ),
    ])
    return { orders: rows, total: countRow?.cnt ?? 0, page, limit, totalPages: Math.ceil((countRow?.cnt ?? 0) / limit) }
  }

  async findOne(orderId: string, userId: string) {
    const order = await this.db.queryOne<any>(
      'SELECT * FROM store.orders WHERE order_id=$1 AND user_id=$2', [orderId, userId],
    )
  ...
  async updateStatus(orderId: string, status: string, note?: string) {
    ...
    if (!order) throw new NotFoundException({ code: 'ORDER_NOT_FOUND' })
    const timeline = [...(order.timeline ?? []), { status, timestamp: new Date(), note: note ?? '' }]
    ...
    if (status === 'shipped' && !order.shipping_email_sent) {
      ...
      await this.mail.sendShippingNotification(user?.email, user?.first_name, updated).catch(() => void 0)
      await this.db.execute('UPDATE store.orders SET shipping_email_sent=true WHERE order_id=$1', [orderId])
    }
    return updated
  }
  ...
  async createOrder(userId: string, dto: CreateOrderDto) { return this.create(userId, dto) }
  ...
  async getUserOrders(userId: string, opts: { page?: number; limit?: number; status?: string }) {
    ...
    if (status) {
      ...
      return { orders: rows, total: ct?.cnt ?? 0, page, limit, totalPages: Math.ceil((ct?.cnt ?? 0) / limit) }
    }
    return this.findUserOrders(userId, page, limit)
  }

  async getOrder(orderId: string, userId: string) { return this.findOne(orderId, userId) }
  ...
  async calculateShipping(sessionId: string, _addressId: string, _userId: string) {
    const cart     = await this.db.queryOne<any>(
      'SELECT items FROM store.carts WHERE session_id=$1', [sessionId],
    )
    const items    = cart?.items ?? []
    const subtotal = items.reduce((s: number, i: any) => s + (i.subtotal ?? 0), 0)
    const shipping = subtotal === 0 ? 0
      : subtotal >= this.freeShipThreshold ? 0
      : this.flatShipCost
    return { subtotal: parseFloat(subtotal.toFixed(2)), shipping, freeShippingThreshold: this.freeShipThreshold }
  }
}
